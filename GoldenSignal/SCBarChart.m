...
-(void)strokeChart
{
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j=0; j<childAry.count; j++) {
            ...
            //改变柱状图的 frame 位置与宽度
            SCBar * bar = [[SCBar alloc] initWithFrame:CGRectMake((j+(_yValues.count==1?0.3:0.05))*_xLabelWidth +i*_xLabelWidth * 0.47, UULabelHeight, _xLabelWidth * (_yValues.count==1?0.4:0.2), chartCavanHeight)];
