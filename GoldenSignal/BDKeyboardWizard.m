...
- (void)requestServiceData {
    ...
    if (data != nil && data.count > 0) {
        ...
        if ([dbAccess tableExists:tableName]) {
            ...
            [dbAccess createTable:tableName withColumns:@"BD_CODE text,TRD_CODE text,SECU_SHT text,PY_SHT text"];
        }
        ...
        if ([database open]) {
            ...
            @try {
                ...
                for (NSDictionary *item in data) {
                    [database executeUpdate:sql, item[@"BD_CODE"], item[@"TRD_CODE"], item[@"SECU_SHT"], item[@"PY_SHT"]];
                }
            }
